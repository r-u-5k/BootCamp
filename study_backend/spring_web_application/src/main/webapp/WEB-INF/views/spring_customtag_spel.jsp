...
request.setAttribute("guest", new Guest(1, "김수미", new Date().toLocaleString(), "guard@naver.com",
		"http://www.google.com", "오늘은목요일", "Spring EL에대해 공부합니다."));
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	<h1>Spring CustomTag ,SPEL</h1>
	<hr />
	<p>
		- Spring CustomTag : Spring에서 정의한태그<br> - SPEL(Spring Expression Language) 
			--> Spring CustomTag(eval)의 속성(expression) 안에서 사용 가능한 표현 언어(식)
	</p>
	<ul>
		...
		<li>변수1(SPEL String Wrapper): <s:eval expression="price2" />, 
			<s:eval expression="name2" />, <s:eval expression="married2" />
		</li>
		<li>변수2(SPEL Bean): <s:eval expression="guest.guest_name" />, 
			<s:eval expression="guest.getGuest_email()" />,
			<s:eval expression="guest.guest_email.toUpperCase()" />
		</li>
		<li>SPEL + EL(문자열리터럴): <s:eval expression="name0.substring(1)" />
			== <s:eval expression="'${name0}'.substring(1)" />
		</li>
		<li>SPEL + EL(숫자리터럴): <s:eval expression="${price0}+3455" /> 
		...
		<li>EL(SCOPE객체에 등록된 빈객체(DTO): ${guest.guest_name.substring(1,3)}
		</li>
		<li>SPEL(SCOPE객체에 등록된 빈객체(DTO): <s:eval expression="guest.guest_name.substring(1,3)" />
		</li>
		<li>클래스생성자호출(EL에서는 호출불가능) 
			<!-- &dollar;{new java.text.DecimalFormat('###,###.0').format(price1)} -->
		</li>
		<li>클래스생성자호출(SPEL에서는 호출가능):
			<s:eval expression="new java.text.DecimalFormat('###,###.0').format(price1)" /><br>
		</li>
		<li>
		@를 이용해서 SpringContainer에 등록된 빈사용가능<br/>
