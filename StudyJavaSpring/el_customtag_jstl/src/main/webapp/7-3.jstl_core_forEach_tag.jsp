	...
	<ul>
		...
		<c:forEach items="${guestList}" var="guest" varStatus="status">
			<c:if test="${status.count % 2 == 1}">
				<li style="color: blue">${guest.guest_no}, ${guest.guest_name} --> ${status.count}</li>
			</c:if>
			<c:if test="${status.count % 2 == 0}">
				<li style="color: red">${guest.guest_no}, ${guest.guest_name} --> ${status.count}</li>
			</c:if>
		</c:forEach>
		
		<li>---------------Map--------------
			<ul>
				<c:forEach items="${guestMap}" var="entry">
					<c:set var="guest" value="${entry.value}" scope="page"/><!-- 변수 선언 -->
					<li>${guest.guest_no}, ${guest.guest_name}</li>
				</c:forEach>
			</ul>
		</li>
		
		<c:set var="tempVar" value="123" scope="page" />
		<li>${tempVar}</li>
